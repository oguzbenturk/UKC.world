...
router.delete('/:id', authenticateJWT, authorizeRoles(['admin']), async (req, res) => {
  ...
  const qTrue = (v) => String(v || '').toLowerCase() === 'true' || v === '1';
  const hardDelete = qTrue(req.query.hardDelete) || qTrue(req.query.force);
  const deleteAllData = qTrue(req.query.deleteAllData);
    ...
    // If deleteAllData is true, delete all related records (for hard delete)
      if (deleteAllData) {
        ...
        const batch1Results = await Promise.all([
          ...
          // Receipts (RESTRICT FK — must delete before user)
          client.query('DELETE FROM bank_transfer_receipts WHERE user_id = $1', [userId]),
        ]);
        ...
        await Promise.all([
          ...
          // Shop order status history — preserve records, just remove user reference
          client.query('UPDATE shop_order_status_history SET changed_by = NULL WHERE changed_by = $1', [userId]),
          // Bank transfer receipts reviewed_by (user_id rows already deleted in batch 1)
          client.query('UPDATE bank_transfer_receipts SET reviewed_by = NULL WHERE reviewed_by = $1', [userId]),
        ]);
