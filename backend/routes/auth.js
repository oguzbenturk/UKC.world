...
router.get('/me', authenticateJWT, async (req, res) => {
  try {
    ...
    console.log('GET /auth/me - Full token data:', JSON.stringify(req.user));
    ...
    try {      // Simplified query with the correct column name
      ...
      if (dbErr instanceof pg.DatabaseError || dbErr.code) {
        console.log('Database connection error detected, using fallback user data');
      }
      
      // Create a fallback user response from the token data
      // This helps the frontend continue working even if DB has issues
      const fallbackUser = {
        id: req.user.id,
...
// Logout endpoint (optional for JWT-based auth, but useful for logging)
