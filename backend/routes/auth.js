...
router.post('/login', authRateLimit, async (req, res) => {
  ...
  if (isAuthCreationDisabled()) {
    logger.warn('Login blocked: DISABLE_LOGIN is enabled');
...
export const authenticateJWT = async (req, res, next) => {
  ...
  try {
    ...
    if (isAuthCreationDisabled()) {
      const allowLogout =
        req.method === 'POST' &&
        (req.path === '/logout' || (req.originalUrl && req.originalUrl.includes('/auth/logout')));
      if (!allowLogout) {
        logger.warn('Session rejected: DISABLE_LOGIN is enabled (existing JWT not accepted)', {
          userId: verified.id,
          method: req.method,
...
router.post('/register', authRateLimit, async (req, res) => {
  if (isAuthCreationDisabled()) {
    logger.warn('Register blocked: DISABLE_LOGIN is enabled');
    return res.status(503).json({
