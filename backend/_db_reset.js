...
// Keeps ONLY:
//   - users table: admin account + all instructor accounts
...
// Roles to always preserve (by role name, not hardcoded UUID)
const PROTECTED_ROLES = ['admin', 'super_admin', 'instructor'];
...
async function main() {
  ...
  try {
    ...
    // ── Step 4: Delete all users except admin/super_admin/instructor ──
    console.log(`── Step 4: Deleting users (keeping ${PROTECTED_ROLES.join(', ')}) ──`);
    ...
    const deleteResult = await client.query(
      `DELETE FROM users
       WHERE role_id NOT IN (SELECT id FROM roles WHERE name = ANY($1::text[]))`,
      [PROTECTED_ROLES]
    );
    ...
    `);
    console.log('  → Admin account(s) reset to clean state\n');
    ...
    if (DRY_RUN) {
      ...
      console.log('║  Admin + instructors + roles + migrations remain ║');
      console.log('╚══════════════════════════════════════════════════╝');
    }
