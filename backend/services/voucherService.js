...
/**
 * Clients often send voucher row UUID after /vouchers/validate; validateVoucher needs the code string.
 * If the value is not a voucher id UUID, it is treated as the code itself.
 */
export async function resolveVoucherLookupCode(voucherIdOrCode) {
  const raw = voucherIdOrCode != null ? String(voucherIdOrCode).trim() : '';
  if (!raw) return null;
  if (VOUCHER_ROW_UUID_RE.test(raw)) {
    const row = await getVoucherById(raw);
    return row?.code != null ? String(row.code).trim() : null;
  }
  return raw;
}
...
export async function getUserRedemptionCount(voucherId, userId) {
  const result = await pool.query(
    `SELECT COUNT(*) as count FROM voucher_redemptions 
     WHERE voucher_code_id = $1 AND user_id = $2 AND status = 'applied'`,
    [voucherId, userId]
  );
...
export async function redeemVoucher({ 
  ...
  const finalAmt =
    finalAmountParam !== undefined && finalAmountParam !== null
      ? Math.max(0, Number(finalAmountParam) || 0)
      : Math.max(0, orig - disc);

  try {
    // Create redemption record (status must match voucher_redemptions CHECK: applied | refunded | cancelled)
