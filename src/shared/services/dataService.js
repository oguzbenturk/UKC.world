...
class DataService {
  ...
  static async deleteUser(id, options = {}) {
    try {
      const params = {};
      // Backend only runs booking/data cleanup inside the hard-delete branch (requires hardDelete=true).
      // CustomerDeleteModal sends force=true — map to hardDelete so DELETE + deleteAllData actually removes rows.
      if (options.force || options.hardDelete) {
