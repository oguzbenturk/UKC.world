...
import { Button, Avatar, Select, Input, Modal, Form, InputNumber, DatePicker } from 'antd';
...
function Instructors() {
  ...
  const openPayModal = (instructor) => {
    ...
    payForm.setFieldsValue({
      amount: bal?.balance > 0 ? bal.balance : 0,
      payment_date: moment(),
          ...
          <UnifiedResponsiveTable 
            ...
            columns={[
              ...
              },
              {
                ...
                render: (_, record) => {
                  const disciplines = getDisciplines(record);
                  if (disciplines.length === 0) return <span className="text-slate-300">—</span>;
                  return (
              ...
              { 
                ...
                render: (_, instructor) => (
                  <div className="flex items-center justify-end gap-1">
                    {isAdmin && balances[instructor.id]?.balance > 0 && (
              ...
              return (
                <div className="grid grid-cols-3 gap-2 mb-5">
                  <div className="rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100/60 p-3 text-center">
                    <div className="text-[10px] uppercase tracking-wider text-blue-500/80 font-medium">Earned</div>
                  ...
                  <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-100/60 p-3 text-center">
                    <div className="text-[10px] uppercase tracking-wider text-emerald-500/80 font-medium">Paid</div>
                    <div className="text-sm font-bold text-emerald-700 mt-0.5">{fmt(bal.totalPaid)}</div>
                  </div>
                  <div className="rounded-xl bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100/60 p-3 text-center">
                    <div className="text-[10px] uppercase tracking-wider text-orange-500/80 font-medium">Owed</div>
                    <div className="text-sm font-bold text-orange-700 mt-0.5">{fmt(bal.balance)}</div>
                  </div>
                </div>
              );
            })()}
