...
const PartnerStep = ({ serviceId, packageData, durationHours, onDone, onBack, ownedPackage, partnerInfo, onProceedToSchedule, serviceName, servicePrice }) => {
  ...
  const lessonName = packageData?.name || serviceName || 'Semi-Private Lesson';
  ...
  const packageDurationHours = packageData?.totalHours || packageData?.duration_hours ||
    packageData?.durationHours || packageData?.hours || (isStandalone ? durationHours : null);
      ...
      <div className="space-y-4">
        <Alert
          ...
          description={isStandalone
            ? "Semi-private lessons are for 2 people. Invite a friend or let us find you a partner."
            : "This package requires at least 2 people. Invite a friend or let us find you a partner."
          }
        />
        ...
        {onBack && (
          <Button
            ...
            className="!mt-2"
          >
            {isStandalone ? 'Back' : 'Back to package'}
          </Button>
        )}
      </div>
    );
    ...
    return (
      <div className="space-y-4">
        <div className="text-center py-1">
          <h3 className="text-base font-bold text-slate-900">Schedule Your {isStandalone ? 'Semi-Private' : 'Group'} Lesson</h3>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3 space-y-1.5">
          <div className="flex justify-between text-sm">
            <Text type="secondary">{isStandalone ? 'Lesson' : 'Package'}</Text>
            <Text strong className="text-right max-w-[60%] truncate">{lessonName}</Text>
          </div>
          <div className="flex justify-between text-sm">
            <Text type="secondary">Price per person</Text>
            <Text strong className="text-green-600">{dualPrice}</Text>
          ...
          {packageDurationHours && (
            <div className="flex justify-between text-sm">
              <Text type="secondary">{isStandalone ? 'Duration' : 'Package hours'}</Text>
              <Text strong className="text-blue-600">{packageDurationHours}h{isStandalone ? '' : ' total'}</Text>
            </div>
          )}
        </div>
