...
// Simple in-memory cache keyed by "startDate|endDate"
const cache = new Map();
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

function getCacheKey(dateRange) {
...
export const useDashboardData = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // If this request was aborted while in-flight, discard results
      ...
      setCache(cacheKey, { summary: summaryRes, analytics: analyticsRes, ops: opsRes, dashboardSummary: dashSummaryRes });
    } catch (err) {
      if (controller.signal.aborted) return;
