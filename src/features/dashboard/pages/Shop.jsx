...
// Category navigation tabs — static, computed once at module scope
...
// Pre-compute nav items with subcategory children (static data, never changes)
...
const ShopPage = () => {
    ...
    // Fetch products for a specific category (or 'all'/'featured') with caching.
    const fetchForCategory = useCallback(async (category) => {
        ...
        try {
            ...
            if (category === 'all' || category === 'featured') {
                const response = await productApi.getProductsByCategory(30);
                ...
                const response = await productApi.getProductsByCategory(500, category);
                if (currentFetchId !== fetchIdRef.current) return;
                if (response.success && response.categories) {
                    Object.values(response.categories).forEach(g => prods.push(...g.products));
                    normalizeProducts(prods);
            ...
            setMasterProductsVersion(v => v + 1);
            setProducts(prods);
            setPagination({ page: 1, total: prods.length });
        } catch (err) {
            if (currentFetchId !== fetchIdRef.current) return;
    ...
    useEffect(() => {
        ...
        if (selectedCategory === 'featured') {
            ...
            if (selectedSubcategory !== 'all') {
                ...
                // Also resolve children via parent field for non-prefix hierarchies
                // e.g. 'accessories' → 'board-bags', 'spare-parts'
