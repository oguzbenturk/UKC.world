...
const ModernCalendar = () => {
  ...
  /** Keeps drawer content stable when the row is removed from `_bookings` immediately after delete (otherwise `booking` becomes null and the Drawer unmounts mid-flow / blocks the next notification click). */
  const [detailBookingSnapshot, setDetailBookingSnapshot] = useState(null);
  const [_showQuickSearch, _setShowQuickSearch] = useState(false);

  // Prefer live row from context; fall back to snapshot if the row was already removed (e.g. delete)
  const selectedBooking = useMemo(
    () =>
      selectedBookingId
        ? _bookings.find((b) => String(b.id) === String(selectedBookingId)) || null
        : null,
    [_bookings, selectedBookingId],
  );

  const bookingForDetailModal = useMemo(() => {
    if (selectedBooking) return selectedBooking;
    if (
      detailBookingSnapshot &&
      selectedBookingId &&
      String(detailBookingSnapshot.id) === String(selectedBookingId)
    ) {
      return detailBookingSnapshot;
    }
    return null;
  }, [selectedBooking, selectedBookingId, detailBookingSnapshot]);

  useEffect(() => {
    if (selectedBooking && selectedBookingId && String(selectedBooking.id) === String(selectedBookingId)) {
