...
import AccommodationUnitEditor from '../components/AccommodationUnitEditor';

const { useBreakpoint } = Grid;

// Amenities options (for table display)
...
function AccommodationUnitsManager() {
  ...
  const handleEditUnit = (unit) => {
    // Parse meta from amenities if stored there
    const amenitiesRaw = unit.amenities;
    let amenitiesList = [];
    let meta = {};
    
    if (Array.isArray(amenitiesRaw)) {
      const metaEntry = amenitiesRaw.find(a => typeof a === 'string' && a.startsWith('__meta__'));
      if (metaEntry) {
        try { meta = JSON.parse(metaEntry.replace('__meta__', '')); } catch {}
        amenitiesList = amenitiesRaw.filter(a => typeof a !== 'string' || !a.startsWith('__meta__'));
      } else {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          const metaEntry = parsed.find(a => typeof a === 'string' && a.startsWith('__meta__'));
          if (metaEntry) {
            try { meta = JSON.parse(metaEntry.replace('__meta__', '')); } catch {}
            amenitiesList = parsed.filter(a => typeof a !== 'string' || !a.startsWith('__meta__'));
          } else {
    ...
    setEditingUnit({ ...unit, amenities: amenitiesList, meta });
    setEditorVisible(true);
  };

  // Save unit from editor (create or update)
  const handleSaveUnit = async (payload, unitId) => {
    if (unitId) {
      await accommodationApi.updateUnit(unitId, payload);
    } else {
  ...
  const UnitCard = ({ unit }) => {
    ...
    const amenities = (Array.isArray(unit.amenities) ? unit.amenities : []).filter(a => typeof a !== 'string' || !a.startsWith('__meta__'));
  ...
  const unitColumns = [
    ...
    },
    {
      ...
      key: 'amenities',
      render: (amenities) => {
        const items = (Array.isArray(amenities) ? amenities : []).filter(a => typeof a !== 'string' || !a.startsWith('__meta__'));
  ...
  return (
    <div className="space-y-4 md:space-y-6 p-4 md:p-6 max-w-7xl mx-auto min-h-screen bg-slate-50">
      ...
      {/* Airbnb-style Accommodation Unit Editor */}
      <AccommodationUnitEditor
