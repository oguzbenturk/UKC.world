...
const useStudentTables = (displayCurrency, formatCurrency, storageCurrency, convertCurrency) => {
  ...
  const formatDualPrice = useMemo(() => (value, baseCurrency) => {
    ...
    if (!showDualCurrency) {
      return formatCurrency(value, txCurrency);
    }
    const converted = convertCurrency(value, txCurrency, displayCurrency);
    return `${formatCurrency(value, txCurrency)} / ${formatCurrency(converted, displayCurrency)}`;
  }, [showDualCurrency, formatCurrency, displayCurrency, convertCurrency]);
  ...
  const membershipColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        key: 'offering_price',
        render: (value, record) => {
          const baseCurrency = record.offering_currency || displayCurrency;
    ...
    () => [
      ...
      },
      {
        ...
        render: (val, record) => {
          ...
          return formatDualPrice(price, record.currency || displayCurrency);
        },
      },
      {
        title: t('student:profile.history.columns.status'),
...
const useStudentDisplayData = (overview, bookings, rentals, businessCurrency) => {
  ...
  const displayCurrency = useMemo(
    () =>
      overview?.student?.preferred_currency ||
      overview?.student?.preferredCurrency ||
      businessCurrency ||
      'EUR',
    [businessCurrency, overview?.student?.preferred_currency, overview?.student?.preferredCurrency],
  );
