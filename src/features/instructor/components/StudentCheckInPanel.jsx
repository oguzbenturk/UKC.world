const StudentCheckInPanel = ({ students = [], loading = false, onSelect }) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6 space-y-4">
      <header>
        <h2 className="text-lg font-semibold text-slate-900">Students to check in</h2>
      ...
      {loading && !students.length ? (
        ...
        <ul className="space-y-3">
          {students.map((student) => (
            <li key={student.studentId} className="rounded-xl border border-slate-200 bg-white px-4 py-3">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button
                  type="button"
                  className="inline-flex items-center rounded-md border border-slate-200 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-50 transition"
                  onClick={() => {
