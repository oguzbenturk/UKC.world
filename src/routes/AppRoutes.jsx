...
/** Staff = any role except outsider, student, trusted_customer (includes custom roles). */
...
/**
 * Stable module-level component. When this was defined inside AppRoutes, every parent
 * re-render (e.g. sidebar toggle) created a new component type and remounted nested
 * routes — dashboard refetched and showed loading again.
 */
function ProtectedRoute({ allowedRoles = [], staffOnly = false, requiredPermissions = [] }) {
  ...
  if (!isAuthenticated) {
    ...
    const isCustomRole = !BUILT_IN_ROLES.includes(user.role?.toLowerCase());
    if (isCustomRole && user.permissions) {
      if (requiredPermissions.length > 0) {
        ...
        if (needsAdmin) {
          ...
          canAccess =
            user.permissions['bookings:read'] === true ||
