...
- `METHOD_PAYMENT_TX_TYPE` — anında-ödenmiş ("Paid" + yöntem) satışların kredi bacağı tipleri: `cash → cash_payment`, `card/credit_card → card_payment`, `transfer/bank_transfer → bank_transfer_payment` (2026-07-23). **Charge+payment pair deseni:** New Membership / New Rental / New Sale formlarında "Paid" seçilirse cüzdan HİÇ hareket etmez — iki zero-`available_delta` satır yazılır: DEBIT charge (`membership_charge` / `rental_charge` / `shop_order_charge`) + CREDIT yöntem-ödemesi. Kredi bacağı `finances.js INCOME_TX_TYPES` ve `dailyOperationsService DAILY_INCOME_TYPES`'ta kayıtlı → gelir BİR kez sayılır; debit tipleri bilerek bu listelerin DIŞINDA (daily ops type-only ABS eşleşmesi çift saymasın diye). Bu satışlarda indirim `skipWalletCredit: true` ile uygulanır (NET tahsil edildi; kredi bedava para olurdu). Bank-transfer onay akışındaki (bookings.js) pair bu desenin öncülüdür.
...
- **`booking_charge_adjustment` fold'u CÜZDAN-BAZLI olmalı (2026-08-05 fix, v0.1.370).** `fetchTransactions` (`walletService.js:1175`) düzenleme kredilerini orijinal `booking_charge` satırına katlar; `adj` LATERAL yalnız `booking_id`'ye göre topluyordu. Grup/semi-private rezervasyonda katılımcı BAŞINA bir adjustment satırı yazıldığı için HER katılımcının kredisi HER katılımcının borcuna katlanıyordu: 2 kişilik €260 ders €200'e düzenlenince iki cüzdana da +€30 yazılır, ama Financial History her birine `-130 + 60 = -70` gösteriyordu (doğrusu -100). `a.user_id = wallet_transactions.user_id` eklendi; `parent`/`has_parent_charge` EXISTS'ine de aynı scope verildi (yoksa A'nın borç satırı B'nin kredisini "katlandı" sayıp listeden düşürürdü). Prodda 12 canlı rezervasyon / 33 satır etkilenmişti; tek-katılımcılı derslerde davranış birebir aynı. **Aynı hatanın öğrenci-portali ikizi de kapatıldı (v0.1.371):** `studentPortalService.js` aynı fold'u yapar ve sorgu predicate'i diğer katılımcının cüzdan satırlarını çekiyordu (47 öğrenci / 200 satır / ~€17.9k); predicate `wt.user_id = $1`'e daraltıldı — ayrıntı [[Student_Portal]].
