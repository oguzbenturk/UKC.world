...
- **`booking_charge_adjustment` fold'u CÜZDAN-BAZLI olmalı (2026-08-05 fix, v0.1.370).** `fetchTransactions` (`walletService.js:1175`) düzenleme kredilerini orijinal `booking_charge` satırına katlar; `adj` LATERAL yalnız `booking_id`'ye göre topluyordu. Grup/semi-private rezervasyonda katılımcı BAŞINA bir adjustment satırı yazıldığı için HER katılımcının kredisi HER katılımcının borcuna katlanıyordu: 2 kişilik €260 ders €200'e düzenlenince iki cüzdana da +€30 yazılır, ama Financial History her birine `-130 + 60 = -70` gösteriyordu (doğrusu -100). `a.user_id = wallet_transactions.user_id` eklendi; `parent`/`has_parent_charge` EXISTS'ine de aynı scope verildi (yoksa A'nın borç satırı B'nin kredisini "katlandı" sayıp listeden düşürürdü). Prodda 12 canlı rezervasyon / 33 satır etkilenmişti; tek-katılımcılı derslerde davranış birebir aynı. **AYNI HATA HÂLÂ AÇIK:** `studentPortalService.js:1265-1286` aynı fold'u yapar ve sorgu predicate'i (`bp.user_id = $1`, satır 852) diğer katılımcının cüzdan satırlarını da çeker → öğrenci portalında hem yanlış tutar hem de başka müşterinin hareket sızıntısı var ([[Student_Portal]]).
