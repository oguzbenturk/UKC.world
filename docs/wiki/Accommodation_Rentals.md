...
- **Per-person varsayılan OFF:** `pricing_per_person` editörde varsayılan kapalı. Açıkken occupancy satırları "kişi başı" olarak yorumlanır — çift sayım yok ama anlamı değişir; UI bunu turuncu uyarıyla belirtir. **Incident (2026-07-06):** prod'da 2 birimde occupancy kişi-başı oranla (2 misafir→70) + per_person=true girilmişti; local per_person'sızdı (2 misafir→70 TOPLAM = 1 misafirden ucuz!). Karar: occupancy satırları artık **TOPLAM** fiyat tutar (Bag Evi 1→100, 2→140, 3→140; Studio 1→90, 2→100), per_person=false; en üst kapasite satırı açıkça girilir ("3 misafir hâlâ 140"). StayAccommodationModal rate ladder artık 1..capacity TÜM misafir sayılarını pickOccupancyRate fallback'iyle numaralandırır; CTA "Book Now"→"Check Availability" (otel "Request Booking" kaldı). Yedek: `accommodation_units_amenities_backup_20260706` tablosu (prod+local).
