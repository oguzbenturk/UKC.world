...
- **Müşteri Faturası (CustomerBillModal + `customerBill/billAggregator.js`) cüzdandan BAĞIMSIZ hesaplanır:** Balance Due = varlıklardan (booking/rental/shop/membership/paket + Adjustments) kurulan Subtotal − kredi-yönlü tx'lerden kurulan Payments received. Cüzdan bakiyesiyle ancak ledger ↔ varlıklar 1:1 ise (All time + tek para birimi) örtüşür. `discount_adjustment*` her yerde atlanır (indirim, discounts tablosundan kalem tutarına zaten işlenir); satılabilir-varlık iadeleri (`member_purchase_refund`, `shop_order_refund`) **entity-linked sayılır ve fatura satırı OLMAZ** (2026-07-16 düzeltmesi — önceden silinen üyeliğin +iadesi −kredi satırı olarak ikinci kez düşülüyordu). TRY hareketleri **bugünkü** kurla çevrilir (kayıt-anı kuru değil) → kur oynadıkça cüzdanla sapma tasarım gereğidir. Test: `tests/unit/frontend/customers/billAggregator.test.js`. Bkz. MEMORY `zeynep_karahan_bill_wallet_gap`.
