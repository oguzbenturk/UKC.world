...
> **Özet:** `plannivo-landing/` ürünün herkese açık pazarlama sitesidir (`plannivo.com`) — ana React/Vite uygulamasından tamamen ayrı, build adımı olmayan, üç dosyalık bağımsız bir statik kod tabanıdır. 2026-07-05 **"Deep ocean dusk"** immersive yeniden tasarımı (sahip talimatıyla KOYU tema — Duotone marka renkleri): zemin `#0A1520` derin okyanus, yapı Antrasit `#4B4F54` (asla metin değil), neon aksan elektrik mavisi `#009EE2`, akış `#005F8F→#009EE2→#5FD9FF`; canvas 2D rüzgar-haritası motoru (akan parlayan streamline'lar, 6-14sn'de gust darbeleri + yatay sprey, alt bantta okyanus salınımı, ufukta tek izinli amber `#FF9E4F`), 4 cam "beach office" kartı (Live Wind kn — canlı fetch + dürüst "REPRESENTATIVE" fallback, Booking Calendar, Customer Wallet, Equipment — data-depth paralaks + gust CustomEvent ile uçurtma-tepkisi yaylı fizik), scroll-driven "camera glide" bölüm geçişleri (@supports view() + IO fallback), sabit ufuk-ilerleme çizgisi + uçurtma imleci, mobilde bütçeli motor + hamburger nav. Bölümler: hero → 02 live conditions → 03 features (+film slotu) → **04 pricing (fiyat RAKAMI YOK — "Priced per school, not per headache", founding-school çerçevesi)** → 05 proof → 06 promise → 07 demo. Demo formu `mailto:hello@plannivo.com`. Dürüstlük anlatısı aynen korunur: 2025, tek müşteri Duotone Urla (UKC), yalnız Iyzico. (Önceki "Sea-glass" açık tema `landing-design/` arşivinde.)

> **Kütüphaneler:** Saf HTML5 + CSS3 (custom properties, scroll-driven animations `view()` + fallback) + vanilla JS canvas 2D motoru ('lighter' additive glow, DPR≤1.5, FPS watchdog degrade, document.hidden pause, prefers-reduced-motion'da tamamen kapalı + statik fallback); Google Fonts (Bricolage Grotesque display, Hanken Grotesk body, JetBrains Mono); nginx static hosting. Build aracı YOK — framework YOK — HARİCİ JS KÜTÜPHANESİ YOK (three.js değil, el yazması motor). Live Wind kartı `https://ukc.plannivo.com/api/weather/pws`'i 3sn timeout'la dener (CORS için backend `security.js` allowedOrigins'e plannivo.com eklendi); HER hatada görünür "REPRESENTATIVE" etiketiyle temsili değere düşer — canlı rozet YALNIZ gerçek fetch'te. Tasarım kaynağı/arşiv: `landing-design/dusk/dusk.html` (tek dosya) + eski `landing-design/variants/`.
...
- `landing-design/variants/` — 2026-07-04 tasarım turunun 4 adayı (`seaglass.html` = ana sayfaya terfi etti, `horizon.html`, `regatta.html`, `goldenhour.html`); her biri kendi içinde tam sayfa (inline CSS/JS). README de `landing-design/README.md`'ye taşındı. **NEDEN dışarıda:** prod sunucu bir git clone'dur ve deploy, nginx'in plannivo.com olarak servis ettiği `plannivo-landing/` klasörüne git-pull yapar — o klasörde track edilen HER dosya public URL olur (2026-07-04'te variants+README bu yolla sızdı, sunucudan silindi ve repo'da dışarı taşındı).
