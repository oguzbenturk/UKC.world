...
- **`components/PwsLiveStation.jsx`** — büyük anlık rüzgar sayısı + durum çipi (aqua/green/yellow/red palet, WCAG AA), yön oku (rüzgarın estiği yöne döner), ve WU'nun tüm ölçtüğü alanlar (gust, dir, temp, feels-like, humidity, dewpoint, pressure, precip, UV). Verdict'in altında **"hype line"** (`utils/hypeLines.js`): canlı ortalama ≥ 15 kt olunca hız bandına göre (15–19 / 19–25 / 25–30 / 30+) heyecanlı bir tek-satır döner; altındaysa sessiz kalır (flat/light günde sadece verdict). Okuma zaman damgasıyla (`live.unixtime`) seed'lenir → satır okuma başına stabil, yeni okumada değişir. İngilizce kopya (kite argosu çevrilmez). İstasyon kapalıyken zarifçe "Live station offline — showing forecast below." gösterir. Footer'da `wunderground.com/dashboard/pws/<stationId>` linki.
...
- **`components/WindHistoryChart.jsx`** — kaydedilmiş canlı-istasyon geçmişi (`wind_history` tablosu → `GET /weather/pws/history`), saf-SVG bant-renkli çizgi. Y-ekseni **0-çapalı ama adaptif**: tavan pencerenin zirvesine (`niceCeil`, gust öncelikli, +2 kt headroom → 5'e yuvarla, min 20) yapışır → normal session dikey alanı doldurur, iniş-çıkışlar keskin okunur (renk hâlâ mutlak şiddeti kodlar; >35 kt artık tavana kırpılmaz). `LABEL_MS` ile eğri üzerine **satır-içi hız etiketleri** (1h'de ~10 dk, 6h'de ~30 dk, 24h'de 2 sa). Gap'ler gerçek boşluk (interpolasyon yok). Kayıt cron'u 5 dk'da bir (`windHistoryRecorderJob`).
- Yardımcı bileşen/util'ler: `SpotCard`, `HourCell`/`HourDetail`, `DayStrip`, `KiteRecommendation`/`SessionVerdict`, `utils/kiteSize.js` (kilo→kanat boyutu), `windBands.js`, `verdict.js`, `hypeLines.js`, `curveGeometry.js`, `historyConfig.js` (`RANGE_MS`/`TICK_MS`/`LABEL_MS`/`GAP_MS`).
