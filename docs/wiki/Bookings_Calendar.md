...
- **Süre değişimi (duration reprice):** Süre değişince:
  ...
  - Saf **cash** rezervasyonda fiyat orantısal yeniden hesaplanır (2h ders 1.5h'e düşünce €60 → €45) — yalnızca çağıran açık fiyat göndermediyse (checkout yalnızca süre gönderir; manuel fiyat düzenlemesi kazanır). **Saatlik ücret rezervasyonun KENDİ fiyatından türetilir (`currentBooking.amount / oldDuration`), servis kataloğundan DEĞİL (2026-08-05 fix, v0.1.370, `bookings.js:5093`).** Eskiden `services.price / services.duration` kullanılıyordu; bu, önceden yapılmış manuel fiyat düzenlemesini checkout'ta sessizce çöpe atıyordu (2h ders €120'ye çekilir → €60/sa olması gerekirken 1.5h checkout katalog €95/sa ile €142.50 yazıyordu). Hiç düzenlenmemiş rezervasyonda `amount == serviceHourly × oldDuration` olduğu için davranış birebir aynı; katalog oranına yalnızca ölçeklenecek geçerli bir fiyat yoksa düşülür.
