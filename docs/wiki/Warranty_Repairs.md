...
- **Public garanti** (`src/features/warranty/pages/`):
  ...
  - `AdminWarrantyListPage.jsx` — admin liste + istatistik kartları (open/last7d/awaiting_customer/with_manufacturer), arama/durum filtresi/sayfalama. `AdminWarrantyDetailModal` (URL `/admin/warranty/:id` ile senkron) ve `AdminWarrantyCreateModal`. `useAdminWarrantyList`/`useAdminWarrantyStats` hook'ları. **Ant `Tabs` ile iki sekme:** "Claims" (garanti listesi) ve "Spare parts" (aynı `SparePartsOrders` bileşenini gömer — yedek parça siparişleri warranty sayfasından da erişilir).
...
- **Yedek parça siparişleri** (`src/features/admin/pages/SparePartsOrders.jsx` + `backend/routes/spareParts.js` + tablo `spare_parts_orders`, migration 009 + **286**) — standalone paylaşılan liste; `/admin/spare-parts` rotası + Repairs "spare-parts" sekmesi + **warranty sayfası "Spare parts" sekmesi** üzerinden erişilir. part_name/quantity/supplier/notes + durum (pending/ordered/received/cancelled) + **ödeme takibi** (migration 286: `payment_status` unpaid/paid, `cost_amount` NUMERIC, `currency` default EUR, `paid_at`). `/api/spare-parts` GET (`status`/`paymentStatus`/`q` filtreleri) + POST + PATCH (ödeme toggle `paid_at`'i senkronlar) + DELETE, hepsi `authenticateJWT`. **UI (2026-07-08 redesign):** açık temalı "service-bay console" — kart başına 3-düğümlü yaşam-döngüsü rayı (duotone-blue #00a8c4; iptal = kesik gül-kırmızı ray), amber/emerald ödeme pill-toggle (kanal ayrımı: mavi=pipeline, amber/yeşil=para), chip filtreler, para-birimi-bazlı Outstanding/Paid readout, satır-içi tutar düzenleme, custom New Order modalı (ChoiceChips). **CSV yerine markalı PDF export**: `src/features/admin/pdf/sparePartsPdfExport.js` — A4 landscape jsPDF+autotable, "DUOTONE PRO CENTER URLA" antrasit bant + mavi bar; [[Proposals_Quotes]] altyapısını yeniden kullanır (`proposals/constants` BRAND + `proposalFonts.embedProposalFont` 6-dil Unicode).
...
- **Video yüklemesinin gerçek tavanı nginx'tedir, multer'da değil (2026-08-06 olayı):** `warrantyMediaService` 500MB/video + 1.5GB/talep vaat ederken her iki nginx katmanı da `client_max_body_size 35m` idi → ~35MB üstü her video **nginx'in HTML 413'ü** ile reddediliyor, istek Express'e hiç ulaşmıyordu. Telefondan çekilen kısa bir klip bile 50MB+ olduğu için "video eklersem talep açılmıyor" şeklinde görünüyordu. Düzeltme: host + container nginx `1600m` (bkz. [[Deployment_Infrastructure]] "Yükleme boyutu zinciri") + `proxy_request_buffering off` + `backend/server.js` `server.requestTimeout = 600_000`. **Kural:** garanti medya limitlerini yükseltirken nginx zincirini de yükselt, yoksa sessizce geri kırılır.
- **Upload hataları `uploadErrorMessage()` ile gösterilir** (`src/features/warranty/constants.js`): dört yüzey de (public submit, staff files, admin create, admin detail) eskiden `err?.response?.data?.error || generic` yapıyordu — gövde JSON **değilse** (nginx 413 HTML'i) veya yanıt hiç yoksa (kopan bağlantı, axios timeout) mesaj jenerik'e çöküyordu. Helper 413/415/429/5xx/timeout/ağ hatalarını insan-okur mesaja çevirir, API JSON'u varsa onu tercih eder.
