...
- **Cüzdan sorgusu YALNIZ `wt.user_id = $1` olmalı (2026-08-05 fix, v0.1.371, `studentPortalService.js:851`).** Predicate eskiden `b.student_user_id`, `r.user_id` ve `bp.user_id` dallarını da OR'luyordu — yani cüzdana değil, **rezervasyona/kiralamaya** göre eşleşiyordu. Paylaşımlı (semi-private/grup) derste her katılımcının AYNI `booking_id` üzerinde kendi cüzdan satırları olduğu için bu dallar **diğer katılımcının borç/kredi satırlarını** çekiyordu: prodda 47 öğrencide 200 yabancı satır (~€17.9k). İki sonuç: (1) müşteri başka müşterinin defterini okuyabiliyordu, (2) `computeTransactionAggregates` bu satırları Total Paid/bakiyeye topluyor ve yukarıdaki fold `bookingId` bazlı olduğu için her katılımcının kredisi her borca katlanıyordu (€260 → €200 düzenlemesinde -100 yerine **-70** — [[Finances_Wallet]]'taki admin hatasının aynısı). `bp` join'i de `AND bp.user_id = $1` ile daraltıldı (fan-out + rastgele `participant_user_id` düzeldi). Başka birinin cüzdanına yazılan bir borç bu öğrencinin bakiyesini zaten hiç oynatmaz; prodda doğrulandı: öğrencinin GERÇEKTEN kendine ait 0 satırı kayboluyor.
