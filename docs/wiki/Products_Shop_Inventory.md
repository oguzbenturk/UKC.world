...
- **Quick-sale nakit/kart indirimi cüzdana alacak POSTALAMAZ (2026-07-16):** `/admin/quick-sale` her ödeme yönteminde `payment_status='completed'` hardcode'lar; `applyDiscount`'un isPaid kapısı bu yüzden nakit/kart satışta da +indirim alacağı basıyordu — brüt cüzdan borcu olmadığından müşteriye **bedava para**. Artık `skipWalletCredit: payment_method !== 'wallet'` geçilir (discounts satırı yine yazılır, sadece cüzdan alacağı atlanır). Bkz. MEMORY `zeynep_karahan_bill_wallet_gap`.
- **`DELETE /:id` artık cüzdanı da kapatır (2026-07-16):** Eskiden sipariş + discounts satırı silinir ama cüzdan tx'lerine DOKUNULMAZDI → cüzdan-ödemeli siparişte müşterinin parası sessizce kalıyor, indirim alacağı ise var olmayan orderId'ye işaret eden **yetim** kalıyordu (staff finans UI'dan elle temizlemek zorundaydı ve satırı kaçırıyordu — Zeynep Karahan +€48 vakası). Şimdi: (1) `reverseOpenDiscountCreditsForEntity` açık indirim alacaklarını ters kayıtla kapatır, (2) `getEntityNetCharges` ile kalan net cüzdan tahsilatı para birimi bazında iade edilir (idempotent). Sadece `payment_status='completed'` iken; cancel edilmiş (refunded) sipariş atlanır.
